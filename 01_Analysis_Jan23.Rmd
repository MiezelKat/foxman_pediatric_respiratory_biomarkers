...
data = read.csv(file = "./data__processed/full-processed-v2.csv", stringsAsFactors = T)

data = data %>%
  mutate(vir_bac_coinfect = factor(as.character(vir_bac_coinfect), levels = c("none", "only_bac", "only_vir", "both"))) %>%
  # filter(any_bacteria != "") %>%
  filter(albumin_ct < 33) %>%
  mutate(any_bacteria = as.factor(as.character(any_bacteria))) 
  # mutate(viral_min_ct = ifelse(is.na(viral_min_ct), 45, viral_min_ct))
...
```{r results="asis"}
data.temp = data
vars_demo <- paste(c("gender", "age", "cxcl10", "il_1b", "viral_min_ct", "albumin_ct", "any_virus", "any_rvp", "any_sars", "any_bacteria"), collapse=" + ")
...
data.mediation.viral = data %>% 
  filter(!is.na(viral_min_ct)) 

data.mediation.viral = set_variable_labels(.data = data.mediation.viral, .labels = as.list(var.map),  .strict = F)
```

Mediation analysis were viral pathogen strength (i.e., lowest viral (either RVP or SARS-CoV) Ct value) mediates the relationship of age on CXCL10 (log-transformed). It is on virus positive samples only ($n =$ `r nrow(data.mediation.viral)`). 

Nomenclature:

-   **dependent variable**: *CXCL10* is the outcome or dependent variable
-   **independent variable**: *age* is the independent variable that we hypothethise has an effect on CXCL10
-   **mediator**: *min. viral Ct* (i.e., the lowest Ct value from the RVP or SARS-CoV panel)

```{r}
model.0 = glm(cxcl10_log~age, data=data.mediation.viral)
...
For this analysis, we filtered participants for anyone who is **virus positive**, i.e., has a positive SARS-CoV or RVP test ($n =$ `r nrow(data.mediation.viral)`).
...
# Mediation Analysis of Age to CXCL10 by SARS-Cov pathogen 'strength' {.tabset}

```{r echo=FALSE}
data.mediation.covid = data %>% 
  filter(!is.na(sars_ct)) 

data.mediation.covid = set_variable_labels(.data = data.mediation.covid, .labels = as.list(var.map),  .strict = F)
```

Mediation analysis were SARS-CoV pathogen strength (i.e., SARS-CoV Ct value) mediates the relationship of age on CXCL10 (log-transformed). It is on SARS-CoV positive samples only ($n =$ `r nrow(data.mediation.covid)`). 

Nomenclature:

-   **dependent variable**: *CXCL10* is the outcome or dependent variable
-   **independent variable**: *age* is the independent variable that we hypothethise has an effect on CXCL10
-   **mediator**: *SARS-CoV Ct*

```{r}
model.0 = glm(cxcl10_log~age, data=data.mediation.covid)
...
dia.data <- c(0, paste0("'", round(model.m$coefficients["age"], 2), stars.pval(coef(summary(model.m))["age","Pr(>|t|)"]), "'"), # effect main on mediator
          0,
          0, 0, 0,
          paste0("'", round(model.y$coefficients["sars_ct"], 2), stars.pval(coef(summary(model.y))["sars_ct","Pr(>|t|)"]), "'"), # mediator on target
          paste0("'", round(model.0$coefficients["age"], 2), stars.pval(coef(summary(model.0))["age","Pr(>|t|)"]), " (", # main on target without mediator
                 round(model.y$coefficients["age"], # main on target with mediator
                       2), stars.pval(coef(summary(model.y))["age","Pr(>|t|)"]), ")'"), 0)
M<- matrix (nrow=3, ncol=3, byrow = TRUE, data=dia.data)
...
For this analysis, we filtered participants for anyone who is **SARS-CoV positive** ($n =$ `r nrow(data.mediation.covid)`).
...
The first indirect effect of the independent variable *age* on the mediator *SARS-CoV Ct* is `r round(model.m$coefficients["age"], 3)`; `r pvalue(coef(summary(model.m))["age","Pr(>|t|)"], add_p = T)`.
...
The effects of *age* and *SARS-CoV Ct* are `r round(model.y$coefficients["age"], 3)` (`r pvalue(coef(summary(model.y))["age","Pr(>|t|)"], add_p = T)`) and `r round(model.y$coefficients["sars_ct"], 3)` (`r pvalue(coef(summary(model.y))["sars_ct","Pr(>|t|)"], add_p = T)`) respectively.

*Note:* This part is supposed to confirm that the mediator affects the dependent variable while controlling for the independent variable. This means, that for a mediation to take place, the mediator must explain more or other parts of the variance in the dependent variable than the independent variable. If the effect of the IV (*age* in our case) is not significant any more, the mediation through the mediator (*SARS-CoV Ct* in our case) is confirmed.
